...
// *what* to insert; this is the only place that knows *where*.
const ANCHOR_MAP = {
  app: {
    file: "app.js",
...
async function fetchComposerTree() {
  ...
  try {
    ...
    throw new Error(`Could not fetch the composer registry: ${error.message}`);
  }
  return tempDir;
}
...
function applyWiring(outDir, accumulator) {
  ...
  for (const [targetKey, target] of Object.entries(ANCHOR_MAP)) {
    ...
    for (const [slot, marker] of Object.entries(target.slots)) {
      if (!content.includes(marker)) continue;
      const lines = slots[slot] || [];
...
export async function composeBackend({ baseKey, moduleKeys, language, outDir, vars }) {
  ...
  if (!index.bases.includes(baseKey)) {
  ...
  try {
    ...
    applyWiring(outDir, accumulator);
